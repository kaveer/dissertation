...
Public Class member_view_immov_property
    ...
    '################################## THEME SETUP #########################################################################
    Private Sub member_view_immov_property_PreInit(sender As Object, e As EventArgs) Handles Me.PreInit
    ...
    Private Sub bind_to_gridview_room_detail()
        Dim con As New SqlConnection("Data Source=kaveer-pc\SQL12HOMEMASTER;Initial Catalog=HomeMaster;Integrated Security=True")
        Dim sql = " select i_room_id as [ref no] , room_name as [Room] , room_length as [Length] , room_width as [Width] , room_note as [Note] from immovable_room as r inner join immovable_property_tbl as p on r.i_property_id = p.i_property_id where r.i_property_id = @id and room_status='active' "
        Dim sqlCmd As New SqlCommand(sql, con)
        sqlCmd.Parameters.AddWithValue("@id", CInt(GridView_active_immov.SelectedRow.Cells(0).Text))
        con.Open()
        Dim sqladap = New SqlDataAdapter(sqlCmd)
